...
function wp_check_password( $password, $hash, $user_id = '' ) {
    ...
    if ( 0 === strpos( $hash, WP_OLD_HASH_PREFIX ) ) {
        ...
        $check = $wp_hasher->CheckPassword( $md_pass, $hash );

        if ( $check && $user_id ) {
            $hash = wp_set_password( $md_pass, $user_id );
        }
    }

    $check = password_verify( $md_pass, $hash );

    return apply_filters( 'check_password', $check, $md_pass, $hash, $user_id );
}
...
function wp_hash_password( $password ) {
    ...
    $options = apply_filters( 'wp_hash_password_options', ['cost'=> 10] );
    $str=iconv(mb_detect_encoding($password, mb_detect_order(), true), "UTF-8", $password);
    // function write_to_console($password) {
    //     $console = $password;
    //    $encoded_pass = $str;
    //     echo "<script>console.log('Console: " . $console . "' );</script>";
    //     echo "<script>console.log('Encoded: " . $encoded_pass . "' );</script>";
    //    }
    //    write_to_console($password);
    return password_hash( md5($str), PASSWORD_DEFAULT, $options );
}
...
function wp_set_password( $password, $user_id ) {
    ...
    global $wpdb;
    $md_pass = md5($password);
    $str=iconv(mb_detect_encoding($md_pass, mb_detect_order(), true), "UTF-8", $md_pass);
    $hash = wp_hash_password($str);
