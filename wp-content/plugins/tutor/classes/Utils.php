...
class Utils {
	...
	public function get_all_courses_ids_by_user( $user_id = 0 ) {
		global $wpdb;
		$user_id    = $this->get_user_id( $user_id );
		// var_dump( $user_id );
		$course_ids = $wpdb->get_col(
	...
	public function get_all_courses_by_user( $user_id = 0, $post_status = 'publish', $offset = 0, $posts_per_page = -1 ) {
		global $wpdb;

		$user_id    = $this->get_user_id( $user_id );
		$course_ids = array_unique( $this->get_all_courses_ids_by_user( $user_id ) );
...
//			),
	...
	public function get_passed( $user_id = 0,$quiz_id = 0 ) {
		
		global $wpdb;
		$em = 0;
		$tm = 0;
		$attempt_list = tutor_utils()->quiz_attempts($quiz_id,$user_id);
		
		if($attempt_list){
		foreach($attempt_list as $key=>$attList){
			if($attList->earned_marks > $em){
				$em = $attList->earned_marks;
			}
			if($attList->total_marks > $tm){
				$tm = $attList->total_marks;
			}
			$quiz_result[$key]['total_marks'] = $attList->total_marks;
			$quiz_result[$key]['earned_marks'] = $attList->earned_marks;
		}
		$passing_grade = tutor_utils()->get_quiz_option($quiz_id, 'passing_grade', 0);
		
		$earned_percentage 	= $em > 0 ? ( number_format( ( $em * 100 ) / $tm ) ) : 0;
		$pass_status =  $earned_percentage >= $passing_grade;
